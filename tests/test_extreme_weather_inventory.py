...
INVENTORY_PATH = ROOT / "data" / "raw" / "extreme_weather_verified" / "verified_extreme_weather_inventory_2013_2025.csv"
...
def test_extreme_weather_inventory_preserves_cross_hazard_verified_rows():
    ...
    assert sum(1 for row in frame if row["hazard_type"] == "flash_flood") == 21
    assert sum(1 for row in frame if row["hazard_type"] == "dust_storm") == 4
    ...
    assert sum(1 for row in frame if row["hazard_type"] == "snowstorm") == 1
